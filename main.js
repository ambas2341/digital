...
// [BARU] LOGIKA KEMBALI KE HOME (/START)
        case "home": {
            ...
            // Mengirim ulang pesan foto (karena tidak bisa edit text ke photo)
...
await bot.telegram.sendMessage(
  ...
  `📦 Produk: ${p.name}`
        ...
        case "fixbuyscript": {
            ...
            reply(`*Mempersiapkan Pembayaran Script...*\nProduk: ${p.name}\nTotal: Rp${toRupiah(amount)}`)

            try {
                let merchantOrderId = "INV-SCR-" + Date.now()
                // ... LOGIKA PAYMENT GATEWAY DUITKU ...
                let rawSignature = DUITKU_MERCHANT_CODE + merchantOrderId + amount + DUITKU_API_KEY
                let signature = crypto.createHash("md5").update(rawSignature).digest("hex")

                let trx = await axios.post("https://passport.duitku.com/webapi/api/merchant/v2/inquiry", {
                  merchantCode: DUITKU_MERCHANT_CODE, paymentAmount: amount, paymentMethod: "SP", merchantOrderId: merchantOrderId,
                  productDetails: p.name, customerVaName: pushname, email: username + "@telegram.com", phoneNumber: "08123456789",
                  itemDetails: [{ name: `${p.name} (x${jumlah})`, price: amount, quantity: 1 }],
                  customerDetail: { firstName: pushname, lastName: "-", email: username + "@telegram.com", phoneNumber: "08123456789" },
                  callbackUrl: "https://eragame.web.id/wejizy/duitku/callback", returnUrl: "https://eragame.web.id/wejizy/duitku/return", 
                  signature: signature, expiryPeriod: 10
                }, { headers: { "Content-Type": "application/json" } })

                let result = trx.data
                if (result.statusCode !== "00") { reply("Gagal: " + result.statusMessage); break }

                let qrPath = `./options/QRIS-${result.reference}.png`
                await QRCode.toFile(qrPath, result.qrString, { color: { dark: "#000000", light: "#ffffff" }, width: 400 })

                await ronzz.replyWithPhoto({ source: qrPath }, {
                  caption: `*🧾 TAGIHAN SCRIPT*\n*📦:* ${p.name}\n*💵 Total:* Rp${toRupiah(amount)}\n\n_Scan QRIS di atas_`,
                  parse_mode: "MARKDOWN",
                  reply_markup: { inline_keyboard: [[{ text: "🔗 Bayar Web", url: result.paymentUrl }], [{ text: "🚫 Batal", callback_data: `cancelbuy ${userId}` }]] }
                })

                db.data.order[user_id] = { id: pid, jumlah: jumlah, chatId: from, ref: result.reference, merchantOrderId: merchantOrderId, type: 'script' }
                ...
                (async () => {
                    ...
                    while (db.data.order[user_id] !== undefined) {
                        ...
                        try {
                            let sigCheck = crypto.createHash("md5").update(DUITKU_MERCHANT_CODE + merchantOrderId + DUITKU_API_KEY).digest("hex")
                            let cek = await axios.post("https://passport.duitku.com/webapi/api/merchant/transactionStatus", { merchantCode: DUITKU_MERCHANT_CODE, merchantOrderId: merchantOrderId, signature: sigCheck }, { headers: { "Content-Type": "application/json" } })

                            if (cek.data.statusCode === "00") {
                                bot.telegram.sendMessage(from, "✅ Lunas! Mengirim file..."); 
                                ...
                                delete db.data.order[user_id]
                                break
                            } else if (cek.data.statusCode !== "01") { bot.telegram.sendMessage(from, "Gagal."); delete db.data.order[user_id]; break }
                        } catch (err) { }
                    }
                })()
...
╰─────╼`
  ...
  bot.command('start', async (ronzz) => {
    ...
    // [BARU] Update tombol jadi 2 baris (Produk Biasa & Script)
    let button = [
      [
