...
// [BARU] LOGIKA KEMBALI KE HOME (/START)
        ...
        case "fixbuy": {
            // [PERBAIKAN DI SINI] Gunakan global.payment
            const orkutConfig = global.payment; 
            const APIGATEWAY_ORKUT = "https://orkut-sand.vercel.app/orderkuota"; 
            
            // Cek jika config payment belum diisi
            if (!orkutConfig) {
                reply("⚠️ Konfigurasi payment belum diset di setting.js");
                return;
            }
            ...
            let baseAmount = priceInt * jumlah
            
            // [LOGIKA ORKUT] Generate angka unik (1-100)
            let fee = Math.floor(Math.random() * 100) + 1; 
            let totalBayar = baseAmount + fee;

            reply(`*Mempersiapkan Pembayaran Orkut...*\nProduk: ${p.name}\nJumlah: ${jumlah}\nMohon tunggu sebentar...`)

            try {
                let merchantOrderId = "ORD-" + Date.now()
                let qrImageUrl = "";

                // 1. REQUEST QRIS KE ORKUT
                try {
                    ...
                    if (!resOrkut.data.status || !resOrkut.data.result) {
                        throw new Error(resOrkut.data.message || "Gagal mengambil data QRIS.");
                    }

                    const dataRes = resOrkut.data.result;
                    qrImageUrl = dataRes.imageqris.url || dataRes.imageqris;

                } catch (err) {
                     console.log("Orkut Create Error:", err.message)
                     reply("❌ Gagal membuat tagihan QRIS. Cek server payment.")
                     break
                }

                // 2. KIRIM INVOICE QRIS
                await ronzz.replyWithPhoto({ url: qrImageUrl }, {
                  caption: `*🧾 TAGIHAN PEMBAYARAN*\n\n` +
                           `*📦 Produk:* ${p.name}\n` +
                           `*💵 Total:* Rp${toRupiah(totalBayar)}\n` + 
                           `_(Termasuk Biaya Admin Rp${fee})_\n\n` +
                           `*⚠️ PENTING:* Qris Expired Dalam 5 Menit\n\n` +
                           `_Menunggu pembayaran..._`,
                  parse_mode: "MARKDOWN",
                  reply_markup: { 
                      inline_keyboard: [
                          [{ text: "🚫 Batalkan Transaksi", callback_data: `cancelbuy ${user_id}` }] 
                      ] 
                  }
                })
                ...
                // 4. POLLING STATUS (CEK MUTASI)
                ;(async () => {
                    let attempts = 0;
                    while (db.data.order[user_id] !== undefined && attempts < 20) {
                        await simple.sleep(15000) 
                        attempts++;

                        if (!db.data.order[user_id]) break;
                        
                        if (Date.now() - db.data.order[user_id].startTime > toMs("5m")) {
                             bot.telegram.sendMessage(from, "⚠️ Pesanan dibatalkan karena waktu habis (5 menit).")
                             delete db.data.order[user_id]
                             break
                        }
                        ...
                        try {
                            ...
                            if (Array.isArray(mutasiList)) {
                                isPaid = mutasiList.some(trx => {
                                    const kredit = parseInt(trx.kredit.toString().replace(/[^0-9]/g, ""));
                                    return kredit === parseInt(totalBayar);
                                });
                            }
                            ...
                            if (isPaid) {
                                ...
                                let teks = `Tanggal: ${new Date().toLocaleString()}\nOrder ID: ${merchantOrderId}\n\n----- AKUN ANDA -----\n`
                                ...
                                const filename = `./options/TRX-${reffId}.txt`;
                                fs.writeFileSync(filename, teks, "utf8")
                                
                                await bot.telegram.sendDocument(from, { 
                                    source: filename, 
                                    filename: `Order-${p.name}.txt` 
                                }, { caption: "Terimakasih sudah berbelanja! 🤝" })
                ...
                reply("Terjadi kesalahan sistem saat memproses order.")
            }
        }
        break
        ...
        case "fixbuyscript": {
            // [CONFIG ORKUT] Gunakan global.payment
            const orkutConfig = global.payment; 
            const APIGATEWAY_ORKUT = "https://orkut-sand.vercel.app/orderkuota"; 
            ...
            // [LOGIKA ORKUT] Tambah kode unik (1-100)
            let fee = Math.floor(Math.random() * 100) + 1; 
            let totalBayar = baseAmount + fee;

            reply(`*Mempersiapkan Pembayaran Script Orkut...*\nProduk: ${p.name}\nTotal: Rp${toRupiah(totalBayar)}\n_Mohon tunggu sebentar..._`)
            ...
            try {
                ...
                try {
                    ...
                    qrImageUrl = dataRes.imageqris.url || dataRes.imageqris;
                     ...
                     console.log("Orkut Script Create Error:", err.message)
                     reply("❌ Gagal membuat tagihan QRIS.")
                     break
                }
                ...
                await ronzz.replyWithPhoto({ url: qrImageUrl }, {
                  caption: `*🧾 TAGIHAN SCRIPT*\n\n` +
                           ...
                           `*💵 Total:* Rp${toRupiah(totalBayar)}\n` + 
                           `_(Termasuk Biaya Admin Rp${fee})_\n\n` +
                           `*⚠️ PENTING:* Expired Qris Dalam 5 Menit\n\n` +
                           `_Menunggu pembayaran..._`,
                  parse_mode: "MARKDOWN",
                  reply_markup: { 
                      inline_keyboard: [
                ...
                db.data.order[user_id] = { 
                    ...
                    merchantOrderId: merchantOrderId, 
                    type: 'script',
                    totalBayar: totalBayar, // Penting untuk mutasi
                    startTime: Date.now()
                }
                ...
                ;(async () => {
                    ...
                    while (db.data.order[user_id] !== undefined && attempts < 20) {
                        ...
                        // Cek Expired
                        if (Date.now() - db.data.order[user_id].startTime > toMs("5m")) {
                             bot.telegram.sendMessage(from, "⚠️ Pesanan Script dibatalkan karena waktu habis.")
                             delete db.data.order[user_id]
                             break
                        }
                        ...
                        try {
                            ...
                            if (isPaid) {
                                bot.telegram.sendMessage(from, "✅ Pembayaran Lunas! Mengirim file script..."); 
                                ...
                                let dataStok = []
                                for (let i = 0; i < jumlah; i++) { 
                                    dataStok.push(db.data.script[pid].stok.shift()) 
                                }
                                ...
                                dataStok.forEach(i => { 
                                    let dAkun = i.split("|"); // Format: Link|Pass|Info
                                    teks += `Link Download: ${dAkun[0]}\nPassword: ${dAkun[1]||'-'}\nInfo: ${dAkun[2]||'-'}\n\n` 
                                })
                                
                                const filename = `./options/TRX-SCR-${reffId}.txt`;
                                fs.writeFileSync(filename, teks, "utf8")
                                
                                await bot.telegram.sendDocument(from, { 
                                    source: filename, 
                                    filename: `Script-${p.name}.txt` 
                                }, { caption: "Terimakasih sudah berbelanja Script! 🤝" })
                ...
                console.log("Error FixbuyScript:", e.message)
...
╰─────╼`
  ...
  bot.command('start', async (ronzz) => {
    ...
    // --- BAGIAN STICKER LAMA DIHAPUS AGAR LANGSUNG MUNCUL MENU DENGAN API ---
    ...
    // 2. Kirim Foto dengan EFEK API (Fire)
    await ronzz.replyWithPhoto({
        source: thumbnail 
    }, {
        caption: teks,
